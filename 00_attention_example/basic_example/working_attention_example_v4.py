...
def main():
    ...
    # Test
    x_test, y_test = task.next_batch()
    x_test = x_test.to(device)
    y_test = y_test.to(device)

    with torch.inference_mode():
        self_attention_weighted_x_test = self_attention_model(x_test)
        y_test_predictions = model(self_attention_weighted_x_test)
        acc = accuracy_fn(y_true=y_test * max_len,
                          y_pred=torch.round(y_test_predictions * max_len),
                          vocab_size=vocab_size)
        # Print training information
